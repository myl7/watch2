#!/usr/bin/env python3
"""Transcribe a video through an OpenAI-compatible speech-to-text API.

Strategy: extract audio (mono 16kHz mp3, tiny payload), upload to whichever
provider has a key. Returns segments in the same shape as transcribe.parse_vtt
so the rest of the pipeline (filter_range, format_transcript) doesn't care
where the transcript came from.

Every provider here speaks the same multipart POST, so adding one is a row in
PROVIDERS rather than a new module.

Pure stdlib — no provider SDKs needed.
"""
...
# Every provider below exposes the same OpenAI /audio/transcriptions shape, so
# a new one is a row here, not a new code path. `usd_per_hour` is list price for
# an hour of audio; it exists to keep the cost ordering of PREFERENCE honest.
PROVIDERS: dict[str, dict[str, str | float]] = {
    ...
    # Whisper segments audio into ~6s spans against Qwen3-ASR's fixed ~27s
    # slices. Worth the drop in Chinese accuracy only when you need timestamps
    # finer than a topic boundary.
    "deepinfra-whisper": {
        "endpoint": "https://api.deepinfra.com/v1/openai/audio/transcriptions",
...
# Order `auto` tries providers in. DeepInfra's Qwen3-ASR leads because it is both
# the cheapest of the three defaults and markedly better on Chinese: on a mixed
# zh/en podcast Whisper dropped all punctuation and mangled proper nouns
# ("ChatGPT" -> "差子就列") that Qwen3-ASR got right.
PREFERENCE = ("deepinfra", "groq", "openai")

# OpenAI whisper-1 and Groq both cap uploads at 25 MB. We target a margin under
# that so multipart framing overhead never pushes a chunk over. At the 64 kbps
# mono mp3 this module produces that is a little under an hour per request.
...
def load_api_key(preferred: str | None = None) -> tuple[str, str] | tuple[None, None]:
    """Return (backend, api_key), walking PREFERENCE until a key turns up.

    If `preferred` names a provider, only that provider's key is considered.
    """
    ...
    order = (preferred,) if preferred is not None else PREFERENCE
    candidates = tuple(
        (str(PROVIDERS[name]["key_env"]), name) for name in order if name in PROVIDERS
    )
...
def _transcribe_file(backend: str, api_key: str, audio_path: Path) -> list[dict]:
    ...
    response = _post_whisper(
        str(provider["endpoint"]), api_key, str(provider["model"]), audio_path
    )
...
def transcribe_video(
    ...
    if not backend or not api_key:
        ...
        raise SystemExit(
            "No ASR API key available. Set DEEPINFRA_API_KEY (preferred), GROQ_API_KEY, "
